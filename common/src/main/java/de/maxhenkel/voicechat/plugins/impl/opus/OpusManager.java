...
import de.maxhenkel.voicechat.voice.client.ClientVoicechatConnectionApi;
import de.maxhenkel.voicechat.voice.client.SoundManagerApi;
...
public class OpusManager {
    ...
    public static boolean opusNativeCheck() {
        ...
        Boolean success = Utils.createSafe(() -> {
            NativeOpusEncoderImpl encoder = new NativeOpusEncoderImpl(SoundManagerApi.SAMPLE_RATE, 1, Application.VOIP);
            encoder.setMaxPayloadSize(SoundManagerApi.MAX_PAYLOAD_SIZE);
            byte[] encoded = encoder.encode(new short[SoundManagerApi.FRAME_SIZE]);
            encoder.resetState();
            encoder.close();

            NativeOpusDecoderImpl decoder = new NativeOpusDecoderImpl(SoundManagerApi.SAMPLE_RATE, 1);
            decoder.setFrameSize(SoundManagerApi.FRAME_SIZE);
    ...
    private static OpusEncoder createNativeEncoder(int mtuSize, Application application) {
        ...
        try {
            NativeOpusEncoderImpl encoder = new NativeOpusEncoderImpl(SoundManagerApi.SAMPLE_RATE, 1, application);
    ...
    public static OpusEncoder createEncoder(OpusEncoderMode mode) {
        int mtuSize = SoundManagerApi.MAX_PAYLOAD_SIZE;
        if (!CommonCompatibilityManager.INSTANCE.isDedicatedServer()) {
            ClientVoicechatApi client = Voicechat.clientInstance.getClient();
            if (client != null) {
                ClientVoicechatConnectionApi connection = client.getConnection();
                if (connection != null) {
        ...
        return new JavaOpusEncoderImpl(SoundManagerApi.SAMPLE_RATE, SoundManagerApi.FRAME_SIZE, mtuSize, application);
    }
    ...
    public static OpusDecoder createDecoder() {
        ...
        return new JavaOpusDecoderImpl(SoundManagerApi.SAMPLE_RATE, SoundManagerApi.FRAME_SIZE);
    }
    ...
    public static boolean useNatives() {
        ...
        if (Voicechat.clientInstance.hasConfig()) {
            return true;
        }
        return Voicechat.clientInstance.shouldUseNatives();
    }
